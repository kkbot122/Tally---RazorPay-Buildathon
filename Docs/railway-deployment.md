...
- `GROQ_API_KEY=<Railway secret variable>`
- `GROQ_MODEL=openai/gpt-oss-120b` (or an approved Groq model)
...
- `AI_MAX_REASONING_ITEMS_PER_REQUEST=1` to isolate provider failures per investigation
- `WEB_ORIGIN=https://<web-public-domain>`

Groq is the only production reasoning provider. Every logical investigation uses its own structured request and remains subject to deterministic verification.
