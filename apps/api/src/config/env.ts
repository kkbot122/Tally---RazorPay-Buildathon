...
export const EnvSchema = z.object({
  ...
  GROQ_MODEL: z.string().trim().min(1).default(DEFAULT_GROQ_REASONING_MODEL),
...
export function workerConfiguration(config: AppConfig) {
  ...
  const constrainedGroq = config.AI_GROQ_TOKENS_PER_MINUTE <= DEFAULT_GROQ_RATE_LIMIT.tokensPerMinute;
  return {
...
export function loadConfig(environment: NodeJS.ProcessEnv = process.env): AppConfig {
  ...
  if (config.NODE_ENV === "production") {
    if (config.GROQ_API_KEY.trim().length === 0) throw new Error("GROQ_API_KEY is required in production");
