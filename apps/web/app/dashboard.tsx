...
export default function Dashboard() {
  ...
  const partialRun = summary?.status === "PENDING" || summary?.status === "PROCESSING";
          ...
          <>
            <section className={`${surface} mb-6 p-[18px] sm:px-5 sm:pb-5`} aria-labelledby="summary-heading">
              ...
              <div className="mt-4 grid grid-cols-2 border-y border-tally-border-subtle sm:grid-cols-3 lg:grid-cols-6">
                {[["Processed", total, "persisted results"], ["Reconciled", reconciled, `${percentage(reconciled, total)} of persisted`], ["Exceptions", explained + discrepancy + unresolved, "needs review or context"], ["Explained outstanding", explained, ""], ["Discrepancies", discrepancy, ""], ["Resolution rate", partialRun ? "—" : percentage(resolved, total), partialRun ? `AI investigations complete: ${summary.completedWorkItems ?? 0} / ${summary.totalWorkItems ?? 0}` : "not unresolved"]].map(([label, value, subtext], index) => <div className={`${metricSeparatorClasses(index)} min-h-[82px] border-tally-border-subtle px-4 py-[13px] first:pl-0`} key={label}><span className={fieldLabel}>{label}</span><strong className="mt-1.5 block text-2xl font-semibold leading-7 tabular-nums">{value}</strong>{subtext !== "" && <span className="text-xs text-tally-ink-muted">{subtext}</span>}</div>)}
              </div>
