...
import ArchitectureExplorer, { HighLevelDesign } from "./architecture-explorer";
...
export default function DocsContent() {
  return <div className="min-h-screen bg-tally-canvas text-tally-ink">
    <header className="sticky top-0 z-20 flex min-h-[52px] flex-wrap items-center gap-x-4 border-b border-tally-border bg-tally-surface px-4 sm:h-[52px] sm:flex-nowrap sm:px-6">
      <nav className="flex w-full flex-wrap items-center gap-x-4 sm:flex-nowrap" aria-label="Primary navigation">
        <Link href="/" className="inline-flex items-center gap-[9px] text-[15px] font-semibold tracking-[-.01em] text-tally-ink no-underline" aria-label="Tally dashboard"><img src="/tally-logo.png" alt="" aria-hidden="true" className="size-5 shrink-0 object-contain" /><span>Tally</span></Link>
    ...
    <main className="mx-auto grid max-w-[1440px] gap-12 px-6 py-10 lg:grid-cols-[220px_minmax(0,760px)] lg:gap-20 lg:px-10 lg:py-14">
      <article className="min-w-0 lg:col-start-2 lg:row-start-1">
        ...
        <Section id="scope-contract" eyebrow="04 / Frozen contract" title="Frozen scope"><p>The workflow is intentionally linear at its boundaries. The complete high-level design is shown below.</p><HighLevelDesign /><Table headers={["Outcome", "Meaning"]} rows={outcomes} /><Callout tone="warning" title="Boundaries that do not move">No foreign-exchange conversion, floating-point money arithmetic, invented identifiers, unsupported relationship shapes, or hidden chain-of-thought are accepted as shortcuts.</Callout></Section>
