...
export default function TracePage({ runId }: { runId?: string }) {
  ...
  useEffect(() => {
    ...
    void getRunTrace(runId).then((nextEvents) => {
      ...
      setError(message.includes("run not found") ? "not-found" : "generic");
      setEvents([]);
      setIsLoading(false);
    });
    return () => { active = false; };
